...
class IODatasetCpu(Dataset):
    def __init__(
        self, flight_paths, window_size=200, check_for_hold=False, check_for_lag=False, input_scaling=False
    ):
        ...
        self._len = 0
        self.check_for_hold = check_for_hold
        self.check_for_lag = check_for_lag
        self.input_scaling = input_scaling
        self.load_data_preproc_every_1_sec(self.check_for_hold, self.check_for_lag, self.input_scaling)
    ...
    def load_data_preproc_every_1_sec(self, check_for_hold=True, check_for_lag=True, input_scaling=False):
        print("Loading data...")
        ...
        for i in tqdm(range(len(self.flight_csv_paths)), desc="Flight#", mininterval=5):
            ...
            if check_for_lag:
                for j in range(1, len(lag_indices)):
                    ...
                    if check_for_hold:
                        non_hold_indices = self.get_non_hold_intervals(
                            df_in, prev, curr
                        )
                ...
                x.extend(df_in.values.tolist())
                y.extend(df_ned[["delta_x", "delta_y", "delta_z"]].values.tolist())  ##

                le += len(x) - (self.window - 1)
                # record current flight's last record index
                self.flight_last_record_index.append(len(x) - 1)

                # record current flight's start record index
                if i != len(self.flight_csv_paths) - 1:
                    self.flight_start_record_index.append(len(x))

                # record flight NED origin
                self.flight_origins.append(
                    df_ned.iloc[0][["x", "y", "z"]].values.tolist()
                )
        ...
        if input_scaling:
            input_mean = torch.mean(self.x, dim=0)
            input_std = torch.std(self.x, dim=0)
            input_std[input_std == 0] = 1.0  # To avoid division by zero
            self.x = (self.x - input_mean) / input_std
            print(f"Input scaling applied. Mean: {input_mean.flatten().tolist()}")
    ...
    def __old_getitem__(self, index):
        if index < 0:
            raise StopIteration("Index can't be less than 0")
