...
class DistillerCpu:
    def __init__(
        ...
        self.optimizer = AdamW(params=self.student_model.parameters(), lr=lr)
        ...
        if wandb_logging:
            ...
            wandb.init(
                project="DeepIO_Model_Distillation",
                name=self.run_name_ts,
                config={
                    "description": description,
                    "model": student_model,
                    "mode_size": sys.getsizeof(student_model),
                    "learning_rate": lr,
                    "epochs": epochs,
    ...
    def load_student_checkpoint(self, filename):
        """
    ...
    def printInfo(self):
        ...
        print("Model size: ", sys.getsizeof(self.student_model))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs), desc=">>>> epoch", mininterval=5):
            ...
            if self.is_stateful:
                self.student_model.reset_states(self.batch_size)
            ...
            for train_data in tqdm(
                ...
                if not self.gpu_resource_info_logged:
                    ...
                    print(f"size of model = {sys.getsizeof(self.student_model)}")
                    print(
                        "---------------------------------------------------------------------------------------"
                    )
                ...
                if self.is_stateful:
                    self.student_model.detach_states()
                ...
                student_loss = self.loss_fn(y_hat, Y[:, :2])
                distillation_loss = self.distillation_loss_fn(y_hat, y_hat_teacher)
                loss = self.alpha * student_loss + (1 - self.alpha) * distillation_loss
                # ! --- End Distillation Step ---

                # loss = self.loss_fn(y_hat, Y[:, :2]) # ! Original line before distillation
